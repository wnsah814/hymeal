...
export default function MenuClient({
  ...
  const [data, setData] = useState<MenuResponse>(initialData);
  const [loading, setLoading] = useState(false);
  const [weekOffset, setWeekOffset] = useState(0);
  const [selectedShop, setSelectedShop] = useState<string | null>(
    initialData.shops.length > 0 ? initialData.shops[0].id : null
  );
  ...
  // Only fetch from API when navigating to a different week
  useEffect(() => {
    if (!mounted) return;
    if (weekOffset === 0) {
      setData(initialData);
  ...
  }, [mounted, weekOffset, fetchMenu, initialData]);
